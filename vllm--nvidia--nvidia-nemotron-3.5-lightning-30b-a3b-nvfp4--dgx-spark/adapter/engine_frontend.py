...
def normalize_vllm_response(payload: bytes) -> bytes:
    """Map vLLM's reasoning field to Let's Infer's unified OpenAI field."""
    return payload.replace(b'"reasoning":', b'"reasoning_content":')
...
class EngineHandler(http.server.BaseHTTPRequestHandler):
    ...
    def _proxy(self) -> None:
        ...
        try:
            ...
            normalize_vllm = (
                self.engine_server.engine == "vllm"
                and self.path.startswith("/v1/")
                and response.status < 400
            )
            event_stream = content_type.startswith("text/event-stream")

            buffered: bytes | None = None
            if normalize_vllm and not event_stream:
                buffered = response.read(MAX_CONTROL_RESPONSE_BYTES + 1)
                if len(buffered) > MAX_CONTROL_RESPONSE_BYTES:
                    raise AdapterError("native engine inference response is too large")
                buffered = normalize_vllm_response(buffered)

            self.send_response(response.status, response.reason)
            for key, value in response.getheaders():
                lowered = key.lower()
                if lowered in HOP_HEADERS | {"date", "server"}:
                    continue
                if normalize_vllm and lowered == "content-length":
                    continue
            ...
            if buffered is not None:
                ...
                self.wfile.flush()
            elif normalize_vllm and event_stream:
                while True:
                    chunk = response.readline()
                    if not chunk:
                        break
                    self.wfile.write(normalize_vllm_response(chunk))
                    self.wfile.flush()
            else:
                while True:
