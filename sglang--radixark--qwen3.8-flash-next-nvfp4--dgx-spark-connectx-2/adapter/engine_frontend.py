...
"""Stable Engine protocol frontend for Qwen3.8 Flash Next.
...
def allocated_backend_port(host: str) -> int:
    """Use the Core-assigned group range; random ports are single-task only."""
    if "LETSINFER_PLACEMENT_GROUP_CONFIG" not in os.environ:
        return _port(host)
    try:
        base = int(os.environ["LETSINFER_PORT_BASE"])
        count = int(os.environ["LETSINFER_PORT_COUNT"])
    except (KeyError, ValueError) as error:
        raise AdapterError("placement-group port allocation is invalid") from error
    if base not in range(1024, 65536) or count < 3 or base + count > 65536:
        raise AdapterError("placement-group port allocation cannot host SGLang")
    return base + 2
...
def run(
    ...
    backend_port = allocated_backend_port(backend_host)
