...
use letsinfer_prefix_store::{
    CaptureRejection, PrefixStore, RegionDescription, StoreConfig, HEADER_BYTES,
};

const FP_A: [u8; 32] = [0xA1; 32];
const FP_B: [u8; 32] = [0xB2; 32];
const SMALL_RECORD_BYTES: u64 = (HEADER_BYTES + (4 + 8 + 4) * 1024) as u64;
...
fn capacity_evicts_oldest_record_first() {
    let dir = tempfile::tempdir().unwrap();
    // Two records fit, while the third must evict one. Keep the expectation
    // derived from the versioned header instead of baking in its old size.
    let store = PrefixStore::open(config(dir.path(), 2 * SMALL_RECORD_BYTES + 4096)).unwrap();
...
fn writer_admission_allows_two_then_rejects() {
    ...
    drop(writer_two);
    assert_eq!(store.statistics().reserved_byte_count, SMALL_RECORD_BYTES);
}
