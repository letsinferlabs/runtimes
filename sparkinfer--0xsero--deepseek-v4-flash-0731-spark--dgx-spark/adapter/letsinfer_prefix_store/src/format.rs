...
pub const HEADER_BYTES: usize = 2 * ALIGNMENT;
const MAGIC: &[u8; 8] = b"LIPFX002";
const VERSION: u32 = 2;
/// Bounded per-record region table: fits fixed-size header encoding and is
/// above the qualified heterogeneous model's 170 per-layer cache regions plus
/// metadata. 192 × (12+8+8+4) bytes = 6,144, which fits the 8 KiB header
/// alongside the fixed fields and retains page-aligned payload offsets.
...
mod tests {
    ...
    #[test]
    fn accepts_heterogeneous_model_region_table() {
        let regions = (0..170)
            .map(|index| RegionDescription {
                name: format!("a{:03}", index),
                byte_count: 4096,
            })
            .chain(std::iter::once(RegionDescription {
                name: "meta".into(),
                byte_count: 128,
            }))
            .collect();
        let mut header = RecordHeader::layout([7; 32], 42, 32_768, regions).unwrap();
        header.tokens_crc32 = 11;
        header.region_crc32 = vec![0; header.regions.len()];
        let mut bytes = vec![0u8; HEADER_BYTES];

        header.encode(&mut bytes).unwrap();
        let decoded = RecordHeader::decode(&bytes).unwrap();

        assert_eq!(decoded.regions.len(), 171);
        assert_eq!(decoded.tokens_offset, HEADER_BYTES);
    }
}
