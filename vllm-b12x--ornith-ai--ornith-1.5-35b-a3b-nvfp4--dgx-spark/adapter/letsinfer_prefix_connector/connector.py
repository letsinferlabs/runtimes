...
Out-of-tree connector for the pinned EUGR/B12X image
(vLLM 0.1.dev20003+gad848fc41.d20260815):
...
class LetsInferPrefixConnector(KVConnectorBase_V1, SupportsHMA):
    ...
    def __init__(
        ...
        engine_identity = extra.get("engine_identity")
        if (
            not isinstance(engine_identity, str)
            or re.fullmatch(r"sha256:[0-9a-f]{64}", engine_identity) is None
        ):
            raise ValueError("Let's Infer connector requires an exact Engine identity")
        self._store_args = (
        ...
        self._max_tokens = int(extra.get("max_tokens", 0))
        if self._max_tokens < self._min_tokens:
        ...
        self._fingerprint = self._compute_fingerprint(
            vllm_config, kv_cache_config, engine_identity
        )
        ...
        self._attention_block_sizes = {
            index: int(group.kv_cache_spec.block_size)
            for index, group in enumerate(self._groups)
            if index in self._attention_group_indices
        }
        if not self._attention_block_sizes or any(
            block_size <= 0
            for block_size in self._attention_block_sizes.values()
        ):
            raise ValueError("Let's Infer requires positive attention block sizes")
        ...
        logger.info(
            ...
            "attn_block_sizes=%s, capture_tokens=%d..%d, "
            "capture_tail_tokens=%d, "
            "fingerprint=%s",
            role.name,
            len(self._groups),
            self._attention_block_sizes,
    ...
    def get_num_new_matched_tokens(
        ...
        if not self._min_tokens <= len(prompt) <= self._max_tokens:
            return 0, False
    ...
    def build_connector_meta(
        ...
        for req_id, scheduled in scheduler_output.num_scheduled_tokens.items():
            ...
            if (
                ...
                and len(prompt) <= self._max_tokens
