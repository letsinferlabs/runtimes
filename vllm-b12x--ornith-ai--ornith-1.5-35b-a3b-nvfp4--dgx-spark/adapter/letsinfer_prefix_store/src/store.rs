...
//!
//! Implements the `PrefixStore` contract: capture admission with a bounded
//! writer count, longest-exact-prefix lookup where a null result is a safe
//! miss, constant-time touch, TTL reclamation, and statistics.
//!
//! Correctness rules:
//!
...
//!
//! The data path uses aligned queue-depth `O_DIRECT` reads, per-path
//! single-flight RAM residency, and one background writer with one queued
//! record. Engine-native block leasing lives in the engine connector because
//! only the engine owns its paged allocator.
