...
_MAX_N = 262144  # covers the Qwen3.8 248320-token draft LM head


def _config(n: int) -> tuple[int, int, int]:
    """Return (rows_per_warp, k_unroll, num_warps) for the active GPU."""
    if torch.cuda.get_device_capability() == (12, 1):
        # DGX Spark: one output row per warp sustains ~244 GB/s on the
        # 248320x5120 draft LM head, versus ~167 GB/s through cuBLAS.
        return (1, 2, 8)
    if n >= 8192:
        return (2, 2, 8)
    return (1, 2, 8)
