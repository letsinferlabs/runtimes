...
uint64_t ds4_gpu_tensor_resident_total(const ds4_gpu_tensor *tensor) {
    return tensor ? ds4_gpu_tensor_bytes(tensor) : 0;
}
...
int ds4_gpu_dspark_markov_top2_tensor(
        ...
        uint32_t                vocab,
        uint32_t                nb) {
    (void)cand_out; (void)alt_out; (void)gap_out; (void)base_logits;
    (void)blk_pos; (void)B; (void)bias; (void)vocab; (void)nb;
    fprintf(stderr, "ds4: dspark_markov_top2 is CUDA-only (DSpark draft)\n");
    return 0;
}
...
int ds4_gpu_dspark_confidence_tensor(
        ...
        uint32_t              B,
        uint32_t              nb) {
    (void)out; (void)dense; (void)markov_embed; (void)model_map;
    (void)model_size; (void)weight_offset; (void)hidden; (void)rank;
    (void)blk_pos; (void)B; (void)nb;
    fprintf(stderr, "ds4: dspark_confidence is CUDA-only (DSpark draft)\n");
    return 0;
}
...
/* Upstream wrapper: optional prefill profiling around the legacy Q8 matmul
 * body (now `_legacy_tensor`).  Driven by DS4_METAL_Q8_PREFILL_PROFILE.
 * Plain pass-through when profiling is off. */
...
int ds4_gpu_attention_decode_raw_batch_heads_tensor(
    ...
    (void)tt_run_pos0;              /* Token-tile hint: CUDA-only. */
    if (!g_initialized && !ds4_gpu_init()) return 0;
...
static int ds4_gpu_encode_router_select(
    ...
    if (hash_mode) {
        ...
        /* PRO support: take upstream's generic n_expert / n_expert_used (was
         * hard-coded 256/6 for Flash).  Append our PC5 substrate trailing
         * args (0u, UINT32_MAX) -- this call is the Metal stub path and
         * never participates in CUDA captured-decode, so the substrate
         * args are legacy values. */
        ok = ds4_gpu_indexer_topk_tensor(selected, score_tensor, n_expert, n_tokens, n_expert_used,
                                         0u, UINT32_MAX) != 0;
    }
    if (!ok) return 0;
...
int ds4_gpu_router_fused_batch_tensor(
    ...
    (void)logits; (void)selected; (void)weights; (void)probs;
    (void)model_map; (void)model_size; (void)weight_offset;
    (void)bias_offset; (void)hash_offset; (void)hash_rows;
    (void)n_expert; (void)n_expert_used; (void)expert_weight_scale;
    (void)n_expert_groups; (void)n_group_used; (void)has_bias;
    (void)hash_mode; (void)x; (void)tokens; (void)in_dim; (void)n_tok;
    return 0;
}
