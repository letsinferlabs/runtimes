...
#define DS4_TOOL_CALL_START "<" DS4_DSML "tool_call>"
#define DS4_TOOL_CALL_END "</" DS4_DSML "tool_call>"
#define DS4_TOOL_CALL_START_SHORT "<" DS4_DSML_SHORT "tool_call>"
#define DS4_TOOL_CALL_END_SHORT "</" DS4_DSML_SHORT "tool_call>"
...
static const char *find_any_tool_start(const char *s) {
    ...
    const char *candidates[] = {
        ...
        strstr(s, "<tool_calls>"),
        strstr(s, DS4_TOOL_CALL_START),
        strstr(s, DS4_TOOL_CALL_START_SHORT),
        strstr(s, "<tool_call>"),
    };
...
static const legacy_dsml_syntax legacy_dsml_syntaxes[] = {
    {DS4_TOOL_CALL_START, DS4_TOOL_CALL_END, DS4_INVOKE_END,
     DS4_PARAM_START, DS4_PARAM_END},
    {DS4_TOOL_CALL_START_SHORT, DS4_TOOL_CALL_END_SHORT, DS4_INVOKE_END_SHORT,
     DS4_PARAM_START_SHORT, DS4_PARAM_END_SHORT},
    {"<tool_call>", "</tool_call>", "</invoke>",
     "<parameter", "</parameter>"},
};

/* Some DS4 generations use the checkpoint's older singular envelope:
 *
 *   <tool_call>
 *   <parameter name="toolName">files_list</parameter>
 *   <parameter name="ability" string="false">{"path":"."}</parameter>
 *   </invoke>
 *
 * It carries one OpenAI function call: toolName is the function name and
 * ability is its complete JSON arguments object.  Accept that exact wire
 * dialect at the runtime boundary so native DSML never leaks into content. */
static bool parse_generated_legacy_tool_call(const char *text,
                                             const char *tool_search,
                                             char **content_out,
                                             char **reasoning_out,
                                             tool_calls *calls) {
    const char *start = NULL;
    const legacy_dsml_syntax *syn = NULL;
    for (size_t i = 0; i < sizeof(legacy_dsml_syntaxes) / sizeof(legacy_dsml_syntaxes[0]); i++) {
        const char *candidate = strstr(tool_search, legacy_dsml_syntaxes[i].start);
        if (candidate && (!start || candidate < start)) {
            start = candidate;
            syn = &legacy_dsml_syntaxes[i];
        }
    }
    if (!start || !syn) return false;

    const char *p = start + strlen(syn->start);
    char *tool_name = NULL;
    char *arguments = NULL;
    buf fields = {0};
    ...
    for (;;) {
        p = skip_ascii_ws(p);
        if (!strncmp(p, syn->invoke_end, strlen(syn->invoke_end))) {
            block_end = p + strlen(syn->invoke_end);
            p = block_end;
            p = skip_ascii_ws(p);
            if (!strncmp(p, syn->end, strlen(syn->end))) block_end = p + strlen(syn->end);
            break;
        }
        if (!strncmp(p, syn->end, strlen(syn->end))) {
            block_end = p + strlen(syn->end);
            break;
        }
        if (strncmp(p, syn->param_start, strlen(syn->param_start))) goto bad;
        const char *tag_end = strchr(p, '>');
        if (!tag_end) goto bad;
        char *tag = xstrndup(p, (size_t)(tag_end - p + 1));
        char *name = dsml_attr(tag, "name");
        ...
        const char *value_start = tag_end + 1;
        const char *value_end = strstr(value_start, syn->param_end);
        if (!value_end) {
            free(name);
            free(is_string);
            goto bad;
        }
        char *raw = xstrndup(value_start, (size_t)(value_end - value_start));
        char *value = is_string && !strcmp(is_string, "true") ?
            dsml_unescape_text(raw) : xstrdup(raw);
        if (!strcmp(name, "toolName")) {
            free(tool_name);
            tool_name = xstrdup(value);
        } else if (!strcmp(name, "ability") || !strcmp(name, "arguments")) {
            free(arguments);
            arguments = json_minify_raw_value(value);
        } else {
            tool_call_json_args_add(&fields, name, value,
        ...
        free(raw);
        free(value);
        p = value_end + strlen(syn->param_end);
    }

    if (!tool_name || !tool_name[0] || !block_end) goto bad;
    if (!arguments) {
        buf wrapped = {0};
        buf_putc(&wrapped, '{');
        buf_puts(&wrapped, fields.ptr ? fields.ptr : "");
        buf_putc(&wrapped, '}');
        arguments = buf_take(&wrapped);
    }
    tool_call tc = {0};
    tc.name = tool_name;
    ...
    tool_calls_push(calls, tc);
    free(calls->raw_dsml);
    calls->raw_dsml = xstrndup(start, (size_t)(block_end - start));
...
static bool legacy_tool_call_is_complete(const char *text) {
    if (!text) return false;
    for (size_t i = 0; i < sizeof(legacy_dsml_syntaxes) / sizeof(legacy_dsml_syntaxes[0]); i++) {
        const char *start = strstr(text, legacy_dsml_syntaxes[i].start);
        if (start && (strstr(start, legacy_dsml_syntaxes[i].invoke_end) ||
                      strstr(start, legacy_dsml_syntaxes[i].end))) return true;
    }
    return false;
}

static size_t generated_invoke_end_len(const char *p, const char *canonical) {
    const char *variants[] = {
        canonical,
        "</" DS4_DSML "oke>",
        "</" DS4_DSML_SHORT "oke>",
    ...
    for (size_t i = 0; i < sizeof(variants) / sizeof(variants[0]); i++) {
        size_t n = strlen(variants[i]);
        if (!strncmp(p, variants[i], n)) return n;
    }
    return 0;
}
...
static bool parse_generated_message_ex(const char *text, bool require_thinking_closed,
    ...
    if (parse_generated_legacy_tool_call(text, tool_search, content_out,
                                         reasoning_out, calls)) return true;
    ...
    for (;;) {
        ...
        bool bare_invoke = false;
        if (strncmp(p, invoke_start, strlen(invoke_start)) != 0) {
            /* The DSML special token can occasionally consume the marker
             * immediately before `invoke`, leaving `invoke name="...">`.
             * The enclosing tool_calls block still makes this unambiguous. */
            if (!strncmp(p, "invoke", 6) &&
                isspace((unsigned char)p[6])) bare_invoke = true;
            else return false;
        }
        const char *tag_end = strchr(p, '>');
        if (!tag_end) return false;
        ...
        while (true) {
            p = skip_ascii_ws(p);
            size_t invoke_end_len = generated_invoke_end_len(p, invoke_end);
            if (invoke_end_len) {
                p += invoke_end_len;
                break;
            }
...
struct server {
    ...
    struct {
        ...
        uint64_t last_use;
        bool     valid;
        /* This bank completed real work and may still own demand-mapped
         * pages even when no reusable text record was produced. */
        bool     resident;
    ...
    /* Let's Infer persistent prefix cache adapter.  NULL keeps the retained
     * server's ordinary warm/native-cache path unchanged. */
    ds4_letsinfer_cache *letsinfer;
    ...
    int  warm_pin_min;
    /* Bound completed shallow request state without limiting live
     * concurrency.  Once a continuous group has fully drained, retain at
     * most this many shallow LRU records and reclaim additional idle banks.
     * Deep records remain until genuine host pressure, when their durable
     * copy is written before reclaim.  Zero disables maintenance. */
    int  warm_shallow_idle_max;
    uint64_t warm_idle_reclaim_floor_bytes;
    job *head;
    job *tail;
    bool stopping;
    int clients;
...
struct job {
    ...
    /* A cache eviction can leave an asynchronous disk capture holding dirty
     * pages after the last active row finishes.  If host-floor admission then
     * blocks the only row, keep it in the continuous FIFO until memory returns,
     * its client disconnects, or shutdown begins.  Worker-owned; the count is
     * observability only and never an admission deadline.  The first attempt's
     * complete placement plan is retained too: recomputing a cold victim on
     * every retry can select a bank the preceding attempt just trimmed and turn
     * temporary pressure into a permanent no-fit loop. */
    uint32_t cont_memory_waits;
    bool cont_memory_queued;
    bool cont_retry_plan_valid;
    int cont_retry_place_bank;
    int cont_retry_fork_bank;
    int cont_retry_n_cached;
    bool cont_retry_target_paid;
    uint64_t cont_retry_seed;
...
static void kv_cache_store_bank(server *s, int bank, const char *reason,
                                int min_committed) {
    if (s->letsinfer && s->batch_ctx && s->warm && bank >= 0 &&
        bank < s->batch_ctx_max_seq && s->warm[bank].valid &&
        s->warm[bank].text && s->warm[bank].len > 0)
        ds4_letsinfer_cache_store_noted(s->letsinfer, s->batch_ctx, bank,
                                     s->warm[bank].text, s->warm[bank].len);
...
decode_again:
    ...
    while (!g_stop_requested && completion < max_tokens &&
        ...
        const int stop_eos = j->req.ignore_eos ? INT_MAX : ds4_token_eos(s->engine);
    ...
    if (j->req.kind == REQ_CHAT && j->req.has_tools &&
        saw_tool_start && !saw_tool_end && legacy_tool_call_is_complete(text.ptr)) {
        saw_tool_end = true;
        finish = "tool_calls";
    }
...
/* Keep the HTTP scheduler ceiling aligned with the engine's fixed 128-slot
 * logical sequence ABI.  Physical request state is admitted separately by
 * the request-sized VMM budget and host-memory floor. */
#define DS4_COALESCE_HARD_MAX 128
...
static void write_batch_completion(server *s, job *j, const ds4_batch_gen_result *r) {
    const int eos = j->req.ignore_eos ? INT_MAX : ds4_token_eos(s->engine);
    char id[96];
    snprintf(id, sizeof(id), "%s-%llu",
             j->req.kind == REQ_CHAT ? "chatcmpl" : "cmpl",
             (unsigned long long)++s->seq);
...
static void generate_batch_jobs(server *s, job **jobs, int n) {
    ...
    for (int i = 0; i < n; i++) {
        ...
        eos_ids[i] = jobs[i]->req.ignore_eos ? INT_MAX : eos;
...
typedef struct {
    ...
    /* A cache-budget rejection while other rows are live is backpressure, not
     * a request failure.  Keep that oldest job in this tiny local FIFO and stop
     * polling the server queue until a live row completes.  The engine then
     * retries it before admitting newer work.  At most one job is normally
     * deferred because memory_blocked stops the next admission immediately;
     * the list form keeps failure cleanup straightforward and preserves FIFO. */
    job    *deferred_head;
    job    *deferred_tail;
    bool    memory_blocked;
    int     deferred;
...
/* Give a concurrent burst one bounded window to reach the FIFO before the
 * continuous engine consumes its primed request. Without this, ordinary HTTP
 * arrival skew can split one intended prefill cohort into waves separated by
 * an entire long-prompt admission. The wait is opt-in, runs only when packed
 * prefill cohorts are enabled, happens once per continuous_generate call, and
 * never holds gen_mu. */
static void cont_wait_initial_peers(cont_sched *cs) {
    const char *cohort = getenv("DS4_CONT_PREFILL_SEGMENTED_COHORT");
    if (!cohort || !cohort[0] ||
        (cohort[0] == '0' && cohort[1] == '\0') || cs->max_seq <= 1)
        return;
    const char *cw = getenv("DS4_SERVER_COALESCE_WAIT_MS");
    int wait_ms = cw ? atoi(cw) : 0;
    if (wait_ms <= 0) return;
    ...
    for (;;) {
        int peers = 0;
        for (job *j = s->head; j && peers < cs->max_seq - 1; j = j->next)
            peers++;
        if (s->stopping || peers >= cs->max_seq - 1) break;
        if (pthread_cond_timedwait(&s->cv, &s->mu, &deadline) != 0) break;
    }
    pthread_mutex_unlock(&s->mu);
}

/* Pace no-active-row memory retries.  This is not an admission override:
 * every retry re-runs the unchanged engine host-floor check.  There is no
 * fixed expiry because cache writeback duration scales with payload size and
 * NVMe state; the queued request itself remains bounded by client lifetime and
 * process shutdown. */
#define DS4_CONT_IDLE_MEMORY_RETRY_US  100000u
...
static void warm_rec_invalidate(server *s, int bank) {
    ...
    if (s->letsinfer) ds4_letsinfer_cache_clear_note(s->letsinfer, bank);
...
static void letsinfer_cache_log_bridge(void *ud, int level, const char *msg) {
    (void)ud;
    const ds4_log_type type = level >= 2 ? DS4_LOG_WARNING
                            : level == 1 ? DS4_LOG_KVCACHE
                                         : DS4_LOG_DEFAULT;
    server_log(type, "ds4-server: %s", msg);
}
...
static void cont_warm_retire(server *s, job *j, const int *tokens, int n,
    ...
    /* Retire establishes the canonical prompt key, but the bank is not
     * settled until its later checkpoint/evict/shutdown capture site. */
    if (s->letsinfer && s->warm[bank].valid &&
        j->req.prompt.v && j->req.prompt.len > 0)
        ds4_letsinfer_cache_note_bank(s->letsinfer, bank,
...
/* A deep cold prompt must not consume a fresh bank merely because one exists.
 * Demand-mapped compressed KV makes each retained deep bank consume unified
 * memory as its frontier grows; an ascending context ladder can therefore
 * cross the host safety floor even though only one request is active.  Let's Infer
 * already persists deep retirements, so for a new deep conversation reuse the
 * least-recently-used idle bank before opening another bank.  Active banks are
 * excluded by occ, preserving real concurrency; shallow records are displaced
 * before deep records, preserving the most expensive live prefix when there is
 * a choice. */
static int warm_cold_reuse_victim_pick(const server *s, const bool *occ,
                                       int prompt_tokens, bool memory_retry) {
    if (!memory_retry &&
        (s->warm_pin_min <= 0 || prompt_tokens < s->warm_pin_min)) return -1;
    int shallow = -1, deep = -1;
    uint64_t shallow_use = UINT64_MAX, deep_use = UINT64_MAX;
    for (int b = 0; b < s->batch_ctx_max_seq; b++) {
        if (occ[b] || !s->warm[b].valid) continue;
        const int committed = ds4_batch_ctx_bank_committed(s->batch_ctx, b, NULL);
        if (committed >= s->warm_pin_min) {
            if (s->warm[b].last_use < deep_use) {
                deep_use = s->warm[b].last_use;
                deep = b;
            }
        } else if (s->warm[b].last_use < shallow_use) {
            shallow_use = s->warm[b].last_use;
            shallow = b;
        }
    }
    return shallow >= 0 ? shallow : deep;
}

static uint64_t server_host_mem_available_bytes(void) {
#ifdef __linux__
...
/* Pick only scheduler-idle retained state.  Shallow state is cheaper to
 * rebuild and goes first; a deep record is eligible only for recovery-floor
 * pressure, after its normal durable-eviction path has run. */
static int warm_idle_reclaim_victim(const server *s, bool shallow_only) {
    int shallow = -1, deep = -1;
    uint64_t shallow_use = UINT64_MAX, deep_use = UINT64_MAX;
    for (int b = 0; b < s->batch_ctx_max_seq; b++) {
        if (!s->warm[b].valid) continue;
        const int committed = ds4_batch_ctx_bank_committed(s->batch_ctx, b, NULL);
        const bool is_deep = s->warm_pin_min > 0 && committed >= s->warm_pin_min;
        if (is_deep) {
            if (!shallow_only && s->warm[b].last_use < deep_use) {
                deep_use = s->warm[b].last_use;
                deep = b;
            }
        } else if (s->warm[b].last_use < shallow_use) {
...
static bool warm_idle_reclaim_one(server *s, int bank, const char *reason) {
    kv_cache_store_bank(s, bank, "bank-evict", s->warm_pin_min);
    const uint64_t freed = ds4_batch_ctx_trim_idle_bank(s->batch_ctx, bank);
    if (freed == 0 && ds4_batch_ctx_bank_committed(s->batch_ctx, bank, NULL) > 0)
        return false;
    warm_rec_invalidate(s, bank);
    s->warm[bank].stored_tokens = 0;
    s->warm[bank].resident = false;
    if (freed > 0)
        server_log(DS4_LOG_GENERATION,
                   "ds4-server: reclaimed idle bank=%d bytes=%llu reason=%s",
                   bank, (unsigned long long)freed, reason);
    return true;
}

/* Run only after ds4_engine_continuous_generate has returned with no parked
 * jobs, while gen_mu is still held.  Therefore every retained bank is idle.
 * The shallow cap bounds ordinary sequential clients; the recovery floor is
 * the second line of defence for a previously concurrent/deep workload. */
static void warm_idle_reclaim(server *s) {
    if (!s->warm || !s->batch_ctx || s->warm_shallow_idle_max <= 0) return;
    /* A completed length-limited/tool request can own mapped pages without a
     * reusable warm record.  Such state has zero cache value: reclaim it
     * before applying the retained-record cap. */
    for (int b = 0; b < s->batch_ctx_max_seq; b++) {
        if (!s->warm[b].resident || s->warm[b].valid) continue;
        const uint64_t freed = ds4_batch_ctx_trim_idle_bank(s->batch_ctx, b);
        s->warm[b].resident = false;
        s->warm[b].stored_tokens = 0;
        if (freed > 0)
            server_log(DS4_LOG_GENERATION,
                       "ds4-server: reclaimed idle bank=%d bytes=%llu reason=no-cache-value",
                       b, (unsigned long long)freed);
    }
    for (;;) {
        int shallow_count = 0;
        for (int b = 0; b < s->batch_ctx_max_seq; b++) {
            if (!s->warm[b].valid) continue;
            const int committed = ds4_batch_ctx_bank_committed(s->batch_ctx, b, NULL);
            if (s->warm_pin_min <= 0 || committed < s->warm_pin_min)
                shallow_count++;
        }
        if (shallow_count <= s->warm_shallow_idle_max) break;
        const int victim = warm_idle_reclaim_victim(s, true);
        if (victim < 0 || !warm_idle_reclaim_one(s, victim, "shallow-cap")) break;
    }
    while (s->warm_idle_reclaim_floor_bytes > 0) {
        const uint64_t available = server_host_mem_available_bytes();
        if (available == 0 || available >= s->warm_idle_reclaim_floor_bytes) break;
        const int victim = warm_idle_reclaim_victim(s, false);
        if (victim < 0 || !warm_idle_reclaim_one(s, victim, "host-floor")) break;
        usleep(100 * 1000); /* bounded driver/unified-memory accounting settle */
    }
}
...
static int cont_disk_victim_pick(const server *s, const bool *occ,
                                 uint32_t rec_tokens) {
    /* A deep durable record already has its authoritative copy on disk.  Reuse
     * an idle resident bank before opening a fresh demand-mapped bank, exactly
     * as a deep cold admission does. */
    if (s->warm_pin_min > 0 && rec_tokens >= (uint32_t)s->warm_pin_min) {
        const int reuse = warm_cold_reuse_victim_pick(s, occ,
                                                       (int)rec_tokens, false);
...
static void cont_warm_pick(server *s, cont_sched *cs, job *j, ds4_cont_request *req,
    ...
    int restored_bank = -1;
    size_t best_len = 0;
    bool live_covers_prompt = false;
    for (int b = 0; b < s->batch_ctx_max_seq; b++) {
        if (occ[b] || !s->warm[b].valid) continue;
        if (plen && s->warm[b].len >= plen &&
            byte_prefix_match(s->warm[b].text, s->warm[b].len, ptext, plen))
            live_covers_prompt = true;
    ...
    if (best < 0 && plen && !live_covers_prompt && s->kv.enabled) {
        int rb = -1;
    ...
    if (best < 0 && plen && !live_covers_prompt && s->letsinfer &&
        j->req.prompt.v && j->req.prompt.len > 0) {
        void *handle = NULL;
        const int tokens = ds4_letsinfer_cache_lookup(
            s->letsinfer, j->req.prompt.v, j->req.prompt.len, &handle);
        if (tokens > 0) {
            const int bank = cont_disk_victim_pick(s, occ, (uint32_t)tokens);
            if (bank < 0) {
                ds4_letsinfer_cache_release(s->letsinfer, handle);
            } else {
                char *text = NULL;
                size_t text_len = 0;
                const int restored = ds4_letsinfer_cache_restore(
                    s->letsinfer, handle, s->batch_ctx, (uint32_t)bank,
                    &text, &text_len);
                if (restored > 0) {
                    warm_rec_invalidate(s, bank);
                    s->warm[bank].text = text;
                    s->warm[bank].len = text_len;
                    s->warm[bank].valid = true;
                    s->warm[bank].last_use = ++s->warm_clock;
                    s->warm[bank].stored_tokens = restored;
                    restored_bank = bank;
                    warm_records_gauge_refresh(s);
                    server_log(DS4_LOG_GENERATION,
                               "ds4-server: letsinfer restore admit bank=%d tokens=%d bytes=%zu",
                               bank, restored, text_len);
                    if (text_len > 0 && text_len < plen &&
                        byte_prefix_match(ptext, plen, text, text_len)) {
                        best = bank;
                        best_len = text_len;
                    }
                }
    ...
    if (best >= 0) {
        ...
        if (hl > 0 && htok) {
            ...
            if (j->warm_prompt.len > hl &&
                ...
                int ft = -1;
                /* A memory-deferred retry must make forward progress.  Forking
                 * preserves the source cache but needs a second bank's mapped
                 * pages -- exactly the allocation the engine just rejected.
                 * Consume the matching source in place on the retry; serving
                 * the waiting request outranks retaining a duplicate warm
                 * prefix, and the durable tier can restore it later. */
                if (s->warm_fork && !memory_retry)
    ...
    if (s->warm_fork_partial && plen) {
        ...
        for (int b2 = 0; b2 < s->batch_ctx_max_seq; b2++) {
            ...
            /* Prefer the record restored for this admission on an equal LCP.
             * Otherwise an older live bank wins by index, the restored bank
             * is duplicated through the partial-fork path, and two identical
             * cache-hit requests can take different numerical paths. */
            if (l > plcp || (l == plcp && b2 == restored_bank)) {
                plcp = l;
                pb = b2;
            }
        }
        ...
        if (pb >= 0 && plcp >= (size_t)s->warm_partial_min) {
            ...
            if (cut >= s->warm_partial_min) {
                ...
                /* Same progress rule as a full-prefix fork: after the engine
                 * has deferred this request for memory, truncate the idle
                 * trunk in place instead of retrying the rejected duplicate. */
                /* A record restored during this admission already has its
                 * durable copy.  Truncate that bank in place instead of
                 * duplicating the restored pages solely to preserve a second
                 * resident copy of the same prefix. */
                int ft = (memory_retry || pb == restored_bank || plcp == plen) ? -1
                         : warm_victim_pick(s, occ, pb, false);
    ...
    /* A memory-deferred retry must use capacity that the preceding completion
     * actually released.  Choosing a never-used bank would repeat the exact
     * projected allocation that caused the engine to defer the request. */
    int target = warm_cold_reuse_victim_pick(s, occ, j->req.prompt.len,
                                              memory_retry);
    if (target < 0) target = warm_victim_pick(s, occ, -1, true);
    if (target >= 0) {
...
static int cont_admit(void *ud, ds4_cont_request *req) {
    cont_sched *cs = ud;
    server *s = cs->s;
    /* The engine declined an admission on its live cache budget.  Do not pull
     * more work into the same known-full memory state; let active rows advance
     * until on_done opens the gate again. */
    if (cs->memory_blocked) return 0;
    ...
    job *j = NULL;
    bool memory_retry = false;
    if (cs->primed) {
        j = cs->primed;          /* the worker's first job (pre-checked to fit). */
        cs->primed = NULL;
    } else if (cs->deferred_head) {
        /* Capacity has been released.  Retry the oldest memory-deferred job
         * before touching the shared queue, retaining strict request order. */
        while (cs->deferred_head && !j) {
            j = cs->deferred_head;
            cs->deferred_head = j->next;
            if (!cs->deferred_head) cs->deferred_tail = NULL;
            j->next = NULL;
            if (client_disconnected(j->fd)) {
                server_log(DS4_LOG_WARNING,
                           "ds4-server: reaped memory-queued request: client disconnected "
                           "before cont retry (prompt=%d tokens never prefilled)",
                           j->req.prompt.len);
                ds4_tokens_free(&j->warm_prompt);
                memset(&j->warm_prompt, 0, sizeof(j->warm_prompt));
                j->cont_retry_plan_valid = false;
                j->cont_retry_target_paid = false;
                j->outcome = JOB_OUT_FAILED;
                job_finish(j);
                j = NULL;
            } else {
    ...
    const bool reuse_retry_plan = memory_retry && j->cont_retry_plan_valid &&
        (j->cont_memory_waits > 0 || j->cont_retry_target_paid);
    if (reuse_retry_plan) {
        /* Retry the exact admission plan that was budget-rejected.  The
         * target/source banks are protected by the engine's trim gate while
         * the request is deferred, so changing placement here can only lose
         * already-paid pages.  A warm effective prompt is job-owned and stays
         * live until admission succeeds, the client leaves, or cleanup runs. */
        if (j->warm_prompt.v && j->warm_prompt.len > 0) {
            req->tokens = j->warm_prompt.v;
            req->n = j->warm_prompt.len;
        }
        req->place_bank = j->cont_retry_place_bank;
        req->fork_bank = j->cont_retry_fork_bank;
        req->n_cached = j->cont_retry_n_cached;
    } else {
        /* A cold first attempt can target a never-used bank while every paid
         * bank is active.  Once a row completes, recompute exactly once so the
         * retry consumes that newly idle capacity instead of protecting the
         * fresh target and letting the engine trim the useful bank.  Warm or
         * already-paid plans stay pinned, as do all no-active-row retries. */
        if (memory_retry) {
            ds4_tokens_free(&j->warm_prompt);
            memset(&j->warm_prompt, 0, sizeof(j->warm_prompt));
        }
        cont_warm_pick(s, cs, j, req, memory_retry);
        j->cont_retry_place_bank = req->place_bank;
        j->cont_retry_fork_bank = req->fork_bank;
        j->cont_retry_n_cached = req->n_cached;
        const int target = req->place_bank - 1;
        j->cont_retry_target_paid = req->n_cached > 0 ||
            (target >= 0 && target < s->batch_ctx_max_seq &&
             ds4_batch_ctx_bank_committed(s->batch_ctx, target, NULL) > 0);
        j->cont_retry_plan_valid = true;
    }
    ...
    if (!memory_retry) j->cont_retry_seed = resolve_job_seed(s, j);
    req->seed        = j->cont_retry_seed;
...
static void cont_resolve_chat(server *s, job *j, cont_stream *st,
    ...
    if (j->req.has_tools && st->saw_tool_start && !st->saw_tool_end &&
        legacy_tool_call_is_complete(st->text.ptr)) {
...
/* on_done: a sequence finished or the engine applied admission backpressure.
 * Otherwise finalize: a streaming job flushes its live SSE stream; a
...
static void cont_on_done(void *ud, void *user, const int *tokens, int n, int finish) {
    cont_sched *cs = ud;
    if (!tokens) {
        /* cont_admit guarantees a valid, in-range request, so an engine decline
         * here is memory backpressure.  Return it to the private FIFO and retry
         * against the unchanged budget/floor instead of leaking a 503.  Active
         * rows provide the next retry event; with no active row, paced retries
         * wait for asynchronous writeback or external pressure to clear.  Only
         * shutdown leaves the job in-flight for the existing cleanup path. */
        int slot = -1, active_other = 0;
        for (int i = 0; i < cs->max_seq; i++) {
            if (cs->inflight[i] == user) slot = i;
            else if (cs->inflight[i]) active_other++;
        }
        cs->memory_blocked = true;
        job *j = user;
        if (slot >= 0 && !g_stop_requested) {
            cs->inflight[slot] = NULL;
            j->cont_bank = -1;
            j->next = NULL;
            if (cs->deferred_tail) cs->deferred_tail->next = j;
            else cs->deferred_head = j;
            cs->deferred_tail = j;
            if (!j->cont_memory_queued) {
                j->cont_memory_queued = true;
                cs->deferred++;
            }
            if (active_other > 0) {
                server_log(DS4_LOG_GENERATION,
                           "ds4-server: memory-queued prompt=%d behind %d active row(s)",
                           j->req.prompt.len, active_other);
            } else {
                if (j->cont_memory_waits != UINT32_MAX) j->cont_memory_waits++;
                /* Nothing is running that can open the gate.  Let staged cache
                 * payloads drain, then have the outer engine loop retry this
                 * same FIFO head against a fresh MemAvailable sample.  Do not
                 * spin, impose a payload-size-dependent deadline, or weaken
                 * the floor. */
                cs->memory_blocked = false;
                if (j->cont_memory_waits == 1u ||
                    j->cont_memory_waits % 100u == 0u)
                    server_log(DS4_LOG_GENERATION,
                               "ds4-server: memory-queued prompt=%d awaiting host-memory headroom (retry=%u)",
                               j->req.prompt.len, (unsigned)j->cont_memory_waits);
                usleep(DS4_CONT_IDLE_MEMORY_RETRY_US);
            }
        }
        return;
    }
    ...
    /* A completion can release mapped cache capacity.  The next engine admit
     * retries the deferred FIFO before accepting newer work. */
    cs->memory_blocked = false;
    cs->served++;
    job *j = user;
    if (cs->s->warm && j->cont_bank >= 0 &&
        j->cont_bank < cs->s->batch_ctx_max_seq)
        cs->s->warm[j->cont_bank].resident = true;
     ...
     * writers below render them as the `timings` block. */
    {
        ...
        if (ds4_cont_last_done_stats(cs->s->batch_ctx, &es) &&
            ...
            /* Report the engine's admission verdict in the client's prompt
             * frame.  The retained engine already exposes authoritative
             * computed/cached counts here; its older server simply never
             * copied them into OpenAI usage for the continuous path. */
            {
                const int prompt = j->req.prompt.len;
                const int computed = (int)es.prefill_computed;
                j->req.cache_write_tokens = computed > prompt ? prompt : computed;
                j->req.cache_read_tokens = prompt - j->req.cache_write_tokens;
            }
        }
    }
...
static void generate_continuous_jobs(server *s, job *first) {
    ...
    cs.served = 0;
    cs.deferred_head = NULL;
    cs.deferred_tail = NULL;
    cs.memory_blocked = false;
    cs.deferred = 0;
    cs.retried = 0;
    ...
    cont_wait_initial_peers(&cs);

    char err[256] = {0};
    pthread_mutex_lock(&s->gen_mu);
    int rc = ds4_engine_continuous_generate(s->batch_ctx, cont_admit, cont_on_token,
                                            cont_on_done, &cs, err, sizeof(err));
    if (rc == 0 && !cs.primed && !cs.deferred_head) {
        bool parked = false;
        for (int i = 0; i < cs.max_seq; i++)
            if (cs.inflight[i]) { parked = true; break; }
        if (!parked) warm_idle_reclaim(s);
    }
    pthread_mutex_unlock(&s->gen_mu);
    ...
    /* An engine failure may strand the private memory queue as well as active
     * slots.  Apply the same bounded fallback policy so every client completes. */
    while (cs.deferred_head) {
        job *jb = cs.deferred_head;
        cs.deferred_head = jb->next;
        jb->next = NULL;
        ds4_tokens_free(&jb->warm_prompt);
        memset(&jb->warm_prompt, 0, sizeof(jb->warm_prompt));
        jb->cont_retry_plan_valid = false;
        jb->cont_retry_target_paid = false;
        cont_stream_discard(jb);
        if (serial_max > 0 && jb->req.prompt.len > serial_max) {
            char msg[192];
            snprintf(msg, sizeof(msg),
                     "Server is temporarily at capacity for a %d-token prompt "
                     "(deep prompts are not served on the serial fallback path); retry shortly",
    ...
    if (rc != 0) {
        ...
        server_log(DS4_LOG_WARNING,
                   ...
                   "memory_queued=%d retried=%d",
                   err[0] ? err : "error", cs.served, fallback,
                   cs.deferred, cs.retried);
    }
    else
        server_log(DS4_LOG_GENERATION,
...
static bool send_token_count_json(server *s, int fd, const char *model,
                                  int prompt_tokens) {
    buf b = {0};
    buf_puts(&b, "{\"object\":\"token_count\",\"model\":");
    json_escape(&b, model);
    buf_puts(&b, ",\"prompt_tokens\":");
    buf_printf(&b, "%d", prompt_tokens);
    buf_puts(&b, "}\n");
    bool ok = http_response(fd, s->enable_cors, 200, "application/json", b.ptr);
    buf_free(&b);
    return ok;
...
/* Let's Infer's benchmark generator needs the exact rendered-chat count without
 * touching inference or cache state.  The public TLS gateway authenticates
 * this request; this backend listener is loopback-only.  Reuse the normal
 * OpenAI parser so counting and serving cannot drift in roles, thinking mode,
 * tool rendering, or special tokens. */
static void handle_token_count(server *s, int fd, const char *body) {
    request req;
    char err[160];
    const int ctx_size = s->serial_boot_ctx;
    if (!parse_chat_request(s->engine, s, body ? body : "", 1,
                            ctx_size, &req, err, sizeof(err))) {
        http_error(fd, s->enable_cors, 400, err);
        return;
    }
    if (req.model_from_request &&
        !server_model_alias_known(req.model ? req.model : "")) {
        request_free(&req);
        http_error(fd, s->enable_cors, 400, "unknown model");
        return;
    }
    send_token_count_json(s, fd, server_model_id_from_engine(s->engine),
                          req.prompt.len);
    request_free(&req);
}
...
static void *client_main(void *arg) {
    ...
    if (!strcmp(hr.method, "POST") && !strcmp(hr.path, "/v1/token-count")) {
        handle_token_count(s, fd, hr.body);
        http_request_free(&hr);
        goto done;
    }
...
static void server_close_resources(server *s) {
    ...
    ds4_letsinfer_cache_close(s->letsinfer);
    s->letsinfer = NULL;
    kv_cache_close(&s->kv);
...
static void usage(FILE *fp) {
    fprintf(fp,
        ...
        "  --mem-floor-gb N\n"
        "      Let's Infer launch compatibility; unified-memory admission is enforced by Watchdog.\n"
        "  --no-update-check\n"
        "      Let's Infer launch compatibility; this retained build has no update checker.\n"
        "  --power N\n"
...
static server_config parse_options(int argc, char **argv) {
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (!strcmp(arg, "-m") || !strcmp(arg, "--model")) {
            ...
            (void)parse_float_arg(need_arg(&i, argc, argv, arg), arg, 0.0f, 1024.0f);
        } else if (!strcmp(arg, "--no-update-check")) {
            /* This retained source predates and does not contain an updater. */
...
int main(int argc, char **argv) {
            ...
            while (try_seq >= 2) {
                if (ds4_batch_ctx_create_fit(s.engine, ds4_session_ctx(s.session), try_seq, cmaxtok,
                    ...
                    if (!(we && we[0] == '0' && we[1] == '\0')) {
                        ...
                        const char *wm = getenv("DS4_SERVER_SHALLOW_IDLE_MAX");
                        s.warm_shallow_idle_max = (wm && wm[0]) ? atoi(wm) : 0;
                        if (s.warm_shallow_idle_max < 0)
                            s.warm_shallow_idle_max = 0;
                        const char *wf = getenv("DS4_SERVER_IDLE_RECLAIM_FLOOR_MB");
                        const long wf_mb = (wf && wf[0]) ? atol(wf) : 0;
                        s.warm_idle_reclaim_floor_bytes = wf_mb > 0
                            ? (uint64_t)wf_mb << 20 : 0;
                        ...
                        s.letsinfer = ds4_letsinfer_cache_open_from_env(
                            s.engine, s.serial_boot_ctx,
                            letsinfer_cache_log_bridge, NULL);
                        if (s.letsinfer && !s.warm_fork_partial)
                            server_log(DS4_LOG_WARNING,
                                       "ds4-server: letsinfer cache is enabled but "
                                       "DS4_SERVER_FORK_PARTIAL=0: exact-repeat "
                                       "prompts cannot consume restored banks");
                    }
                    server_log(DS4_LOG_DEFAULT,
    ...
    } else if (s.letsinfer && s.batch_ctx && s.warm) {
        for (int b = 0; b < s.batch_ctx_max_seq; b++)
            if (s.warm[b].valid && s.warm[b].text && s.warm[b].len > 0)
                ds4_letsinfer_cache_store_noted(s.letsinfer, s.batch_ctx, b,
                                              s.warm[b].text, s.warm[b].len);
    }
    server_close_resources(&s);
    return 0;
...
static void test_parse_legacy_singular_dsml_tool_call(void) {
    const char *generated =
        "<think>list the directory</think>\n"
        DS4_TOOL_CALL_START "\n"
        DS4_PARAM_START " name=\"toolName\" string=\"true\">files_list" DS4_PARAM_END "\n"
        DS4_PARAM_START " name=\"ability\" string=\"false\">{\"path\": \".\"}" DS4_PARAM_END "\n"
        DS4_INVOKE_END;
    char *content = NULL;
    char *reasoning = NULL;
    tool_calls calls = {0};
    TEST_ASSERT(find_any_tool_start(generated) != NULL);
    TEST_ASSERT(legacy_tool_call_is_complete(generated));
    TEST_ASSERT(parse_generated_message_ex(generated, true, &content, &reasoning, &calls));
    TEST_ASSERT(reasoning && !strcmp(reasoning, "list the directory"));
    TEST_ASSERT(content && content[0] == '\0');
    TEST_ASSERT(calls.len == 1);
    TEST_ASSERT(calls.v[0].name && !strcmp(calls.v[0].name, "files_list"));
    TEST_ASSERT(calls.v[0].arguments && !strcmp(calls.v[0].arguments, "{\"path\":\".\"}"));
    TEST_ASSERT(calls.raw_dsml && strstr(calls.raw_dsml, DS4_TOOL_CALL_START));
    free(content);
    free(reasoning);
    tool_calls_free(&calls);
}

static void test_repair_dsml_missing_inner_marker(void) {
    const char *generated =
        "choose the tool</think>\n\n"
        DS4_TOOL_CALLS_START "\n"
        "invoke name=\"files_list\">\n"
        DS4_PARAM_START " name=\"path\" string=\"true\">." DS4_PARAM_END "\n"
        "</" DS4_DSML "oke>";
    buf repaired = {0};
    TEST_ASSERT(try_repair_dsml(generated, strlen(generated), &repaired));
    char *content = NULL;
    char *reasoning = NULL;
    tool_calls calls = {0};
    TEST_ASSERT(parse_generated_message_ex(repaired.ptr, false,
                                           &content, &reasoning, &calls));
    TEST_ASSERT(reasoning && !strcmp(reasoning, "choose the tool"));
    TEST_ASSERT(content && content[0] == '\0');
    TEST_ASSERT(calls.len == 1);
    TEST_ASSERT(calls.v[0].name && !strcmp(calls.v[0].name, "files_list"));
...
static void test_token_count_response_is_count_only(void) {
    server s;
    memset(&s, 0, sizeof(s));
    int sv[2];
    TEST_ASSERT(socketpair(AF_UNIX, SOCK_STREAM, 0, sv) == 0);
    ...
    TEST_ASSERT(send_token_count_json(&s, sv[0], "fixture-model", 32768));
    shutdown(sv[0], SHUT_WR);
    char *out = read_socket_text(sv[1]);
    TEST_ASSERT(strstr(out, "HTTP/1.1 200 OK") != NULL);
    TEST_ASSERT(strstr(out, "\"object\":\"token_count\"") != NULL);
    TEST_ASSERT(strstr(out, "\"model\":\"fixture-model\"") != NULL);
    TEST_ASSERT(strstr(out, "\"prompt_tokens\":32768") != NULL);
    TEST_ASSERT(strstr(out, "token_ids") == NULL);
    free(out);
    close(sv[0]);
    close(sv[1]);
}
...
static void ds4_server_unit_tests_run(void) {
    ...
    test_parse_legacy_singular_dsml_tool_call();
    test_repair_dsml_missing_inner_marker();
    ...
    test_token_count_response_is_count_only();
